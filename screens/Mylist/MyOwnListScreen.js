...
export default class MyOwnListScreen extends React.Component {
  ...
  onCategorySelect = (p_catName) => {
    ...
    this.requestMyOwnList(this.album_id, this.state.categoryItems[this.state.beforeCatIdx].categoryName, this.selectedSubCatName, 0)
  }
  ...
  render() {
    return (
      <KeyboardAvoidingView style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', }} behavior="padding" enabled   /*keyboardVerticalOffset={100}*/>
        ...
        />
        <Toast ref='toast' />
        <TopbarWithBlackBack title={this.album_title} onPress={() => {
        ...
        <ScrollView style={{ flex: 1, flexDirection: 'column' }} keyboardDismissMode="on-drag"
          ...
          <View>
            ...
            <View style={[MyStyles.padding_h_5, MyStyles.padding_v_main, { flex: 1 }]}>
              <MyAppText style={{ color: Colors.primary_dark, fontSize: 14, marginLeft: 10, fontWeight: "500" }}>Product({this.state.product_list_result_data.product_list.length})</MyAppText>
              <FlatGrid
  ...
  requestMyOwnList(p_album_id, p_category, p_sub_category, p_offset) {
    console.log("category= " + p_category);
    console.log("p_sub_category = " + p_sub_category)
    this.setState({
      isLoading: true,
    });
    return fetch(Net.user.myOwnList, {
      method: 'POST',
      headers: {
