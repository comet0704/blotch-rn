...
export default class SearchMainScreen extends React.Component {
  ...
  componentDidMount() {
    this.requestBestList(
      this.state.categoryItems[this.state.beforeCatIdx].categoryName,
    ...
    keyword && this._ocrResultRender(keyword);
  }
  ...
  _ocrResultRender = async keyword => {
    await this.setState({
      query: '',
      searchBoxFocused: true
    });
    this.requestSuggestionList(keyword);
    await this.setState({
      query: keyword,
      searchWord: keyword,
      searchBoxFocused: true
  ...
  getRecentSearchWords() {
    AsyncStorage.getItem(
      MyConstants.ASYNC_PARAMS.recent_search_words,
      (err, result) => {
        if (result != null) {
  ...
  addRecentSearchWords(p_keyword) {
    if (
      p_keyword == '' ||
      this.state.recentSearchWords.indexOf(p_keyword) >= 0
    ) {
      return;
    }
    AsyncStorage.getItem(
      ...
      (err, result) => {
        ...
        if (result != null) {
          ...
          if (w_keywordList.length > 7) {
            // 최근검색어가 7개 이상이면 앞선검색어 삭제
        ...
        w_keywordList.forEach((element, index) => {
          if (index == 0) {
            ...
            settingvalue =
              settingvalue + Common.SEARCH_KEYWORD_SPLITTER + element;
          }
        });
  ...
  onCategorySelect = p_catName => {
    ...
    const index = categoryItems.findIndex(
      item => item.categoryName === p_catName
    );
    ...
    if (this.state.categoryItems[index].sub_category.length > 0) {
      this.selectedSubCatName = this.state.categoryItems[
        index
      ].sub_category[0].name;
    } else {
      this.selectedSubCatName = '';
          ...
          {this.state.categoryItems.map(item => (
            <View key={item.categoryName} style={{ marginRight: 10 }}>
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => {
                  this.onCategorySelect(item.categoryName);
                }}
                style={[MyStyles.category_image_container]}
              >
                {item.is_selected ? (
                  <Image
                    source={require('../../../assets/images/ic_gradient_bg.png')}
                    style={[MyStyles.background_image]}
                  />
                ) : null}
                {item.is_selected ? (
  ...
  renderSubCategory(p_categoryIndex) {
    return this.state.categoryItems[p_categoryIndex].sub_category.length > 0 ? (
      <View style={{ backgroundColor: 'white' }}>
        <View>
          <View style={MyStyles.tabbar_button_container}>
            {this.state.categoryItems[p_categoryIndex].sub_category.map(
              (item, index) => (
                <TouchableOpacity
                  activeOpacity={0.8}
                  key={item.name}
                  style={
                    item.is_selected
                      ? MyStyles.tabbar_button_selected
                      : MyStyles.tabbar_button
                  }
                  onPress={() => {
                    this.selectedSubCatName = item.name;
                    categoryItems = this.state.categoryItems;
                    categoryItems[p_categoryIndex].sub_category.map(
                      item => (item.is_selected = false)
                    );
                    categoryItems[p_categoryIndex].sub_category[
                      index
                    ].is_selected = true;
                ...
                >
                  <MyAppText
                    style={
                      item.is_selected
                        ? MyStyles.tabbar_text_selected
                        : MyStyles.tabbar_text
                    }
                  >
                    {item.name}
                  </MyAppText>
        ...
        <LinearGradient colors={['#eeeeee', '#f7f7f7']} style={{ height: 6 }} />
      </View>
    ) : null;
  }
  ...
  findKeyword(query) {
    ...
    if (query == '') {
      return this.state.recentSearchWords.filter(
        item =>
          item
            .toLowerCase()
            .indexOf(query == null ? '' : query.toLowerCase()) >= 0
      );
    } else {
      return this.state.autocompleteWords.filter(
        item =>
          item
            .toLowerCase()
            .indexOf(query == null ? '' : query.toLowerCase()) >= 0
  ...
  goSearchResultScreen() {
    ...
    this.props.navigation.navigate('SearchResult', {
      [MyConstants.NAVIGATION_PARAMS.search_word]: this.state.searchWord,
      [MyConstants.NAVIGATION_PARAMS.backCallbackfromSearchResult]: this
        .backCallbackfromSearchResult
    });
  }
      ...
      <View style={{ flex: 1, backgroundColor: Colors.color_f2f2f2, marginTop: MyConstants.STATUSBAR_HEIGHT }}>
        ...
        <View style={[MyStyles.searchBoxCommon, MyStyles.bg_white, { marginTop: 0 }]}>
          ...
          {this.state.searchBoxFocused ? (
            <View style={[MyStyles.searchBoxCover, { marginRight: 15 }]}>
              <Image
                source={require('../../../assets/images/ic_search_box_bg1.png')}
                style={MyStyles.background_image_stretch}
              />
              <Image style={{ flex: 1 }} />
              {/* OCR */}
        ...
        <Autocomplete
          ...
          listContainerStyle={[
            { top: 7 },
            this.state.searchBoxFocused
              ? { left: -65, width: this.ScreenWidth + 50 }
              : { left: -50, width: this.ScreenWidth + 20 }
          ]}
          ...
          renderItem={keyword => (
            ...
            >
              ...
              >
                ...
                <MyAppText style={{ color: Colors.primary_purple }}>
                  {query}
                </MyAppText>
                {keyword.substring(
                  keyword.toLowerCase().indexOf(query.toLowerCase()) +
        ...
        {this.state.searchBoxFocused ? null : (
          <View style={{ flex: 1 }}>
            ...
            >
              <View
                style={[
                  MyStyles.padding_h_5,
                  MyStyles.padding_v_main,
                  { flex: 1 }
                ]}
              >
                <MyAppText
                ...
                <FlatGrid
                  ...
                  renderItem={({ item, index }) => (
                    <ProductBestItem item={item} index={index} this={this} />
                  )}
                />
              </View>
            </ScrollView>
  ...
  requestBestList(p_category, p_sub_category, p_offset) {
    ...
    return fetch(Net.product.bestList, {
      ...
      .then(responseJson => {
        ...
        if (p_offset == 0) {
          // 카테고리 선택했을대 offset값을 0에서부터 검색해야 함.
          this.offset = 0;
        }
        this.offset += responseJson.result_data.best_list.length;
        if (
          responseJson.result_data.best_list.length < MyConstants.ITEMS_PER_PAGE
        ) {
